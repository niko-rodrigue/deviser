...
public class Input extends AbstractNamedSBase implements UniqueNamedSBase, CallableSBase {
   ...
   *  
   * @return {@code true} if qualitativeSpecies was set before, otherwise {@code false}.
   */
  public boolean unsetQualitativeSpecies() {
    if (isSetQualitativeSpecies()) {
      String oldQualitativeSpecies = qualitativeSpecies;
      qualitativeSpecies = null;
      firePropertyChange(QualConstants.qualitativeSpecies, oldQualitativeSpecies, qualitativeSpecies);
      return true;
    }
    return false;
  }
   ...
   *  
   * @return {@code true} if transitionEffect was set before, otherwise {@code false}.
   */
  public boolean unsetTransitionEffect() {
    if (isSetTransitionEffect()) {
      TransitionInputEffect oldTransitionEffect = transitionEffect;
      transitionEffect = null;
      firePropertyChange(QualConstants.transitionEffect, oldTransitionEffect, transitionEffect);
      return true;
    }
    return false;
  }
   ...
   *  
   * @return {@code true} if thresholdLevel was set before, otherwise {@code false}.
   */
  public boolean unsetThresholdLevel() {
    if (isSetThresholdLevel()) {
      Integer oldThresholdLevel = thresholdLevel;
      thresholdLevel = null;
      firePropertyChange(QualConstants.thresholdLevel, oldThresholdLevel, thresholdLevel);
      return true;
    }
    return false;
  }
    ...
    if (!isAttributeRead) {
      ...
      if (attributeName.equals(QualConstants.sign)) {
        try {
          ...
          throw new SBMLException("Could not recognized the value '" + value + "' for the attribute " + QualConstants.sign
            + " on the 'Input' element.");
        }

        attributeName.equals(QualConstants.qualitativeSpecies);
        setQualitativeSpecies(value);
        attributeName.equals(QualConstants.transitionEffect);
        try {
