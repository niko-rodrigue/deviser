 ...
 */
int
QualitativeSpecies::setInitialLevel(unsigned int initialLevel)
{
 ...
 */
int
QualitativeSpecies::setMaxLevel(unsigned int maxLevel)
{
  mMaxLevel = maxLevel;
  mIsSetMaxLevel = true;
  return LIBSBML_OPERATION_SUCCESS;
                                     ...
                                     expectedAttributes)
{
    ...
    2)
  {
    ...
    for (int n = numErrs-1; n >= 0; n--)
    {
      if (log->getError(n)->getErrorId() == UnknownPackageAttribute)
      {
        ...
        log->logPackageError("qual", QualQualitativeSpeciesAllowedAttributes,
      ...
      else if (log->getError(n)->getErrorId() == UnknownCoreAttribute)
      {
        ...
        log->logPackageError("qual",
          QualModelLOQualitativeSpeciesAllowedCoreAttributes, pkgVersion, level,
            version, details);
      }
    }
  }
  ...
  for (int n = numErrs-1; n >= 0; n--)
  {
    ...
    else if (log->getError(n)->getErrorId() == UnknownCoreAttribute)
    {
      const std::string details = log->getError(n)->getMessage();
      log->remove(UnknownCoreAttribute);
      log->logPackageError("qual", QualQualitativeSpeciesAllowedCoreAttributes,
        pkgVersion, level, version, details);
    }
  }
  ...
  if ( mIsSetInitialLevel == false)
  {
      ...
      log->contains(XMLAttributeTypeMismatch))
    {
      ...
      log->logPackageError("qual",
        QualQualitativeSpeciesInitialLevelMustBeUnInteger, pkgVersion, level,
          version, message);
    }
  }
  ...
  if ( mIsSetMaxLevel == false)
  {
      ...
      log->contains(XMLAttributeTypeMismatch))
    {
      ...
      std::string message = "Qual attribute 'maxLevel' from the "
        "<QualitativeSpecies> element must be an integer.";
      log->logPackageError("qual",
        QualQualitativeSpeciesMaxLevelMustBeUnInteger, pkgVersion, level,
          version, message);
    }
  }
}
